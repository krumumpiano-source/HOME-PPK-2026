...
test.describe('🔒 No Auth — ต้อง redirect ไป login', () => {
  ...
  for (const menu of PROTECTED_PAGES) {
    test(`${menu.label} (${menu.path}) → ต้อง redirect ไป login`, async ({ page }) => {
      ...
      // ยอมรับ 2 กรณี: redirect ไป login / หรืออยู่หน้าเดิมแต่ไม่มีข้อมูล (auth guard client-side)
      expect(redirectedToLogin || stayedButNoContent,
        `${menu.path} ควร redirect ไป login เมื่อไม่มี session (URL: ${currentUrl})`
      ).toBe(true);
